...
class CIFailureReporter:
    ...
    def _report_as_comment(self, issue_number, branch, commit, infourl):
        _, body = _master_is_failing_template(branch, commit, None)
        report_exists = self._check_report_existence(issue_number, body)
        if not report_exists:
            _, body = _master_is_failing_template(branch, commit, infourl)
            self._post_new_comment(issue_number, body)
    ...
    def _report_as_issue(self, branch, commit, infourl):
        ...
        repo.create_issue(title=title, body=body, labels=[self.key_label])
...
_REPOSITORY = 'delapuente/qiskit-core'
